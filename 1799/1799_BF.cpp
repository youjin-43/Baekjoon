...
class Board{
    ...
    Board(int n,string name){
        this->name = name;
        cout<<this->name<<" 보드 생성!"<<endl;
        size=n;
        board = new int*[size];
        for(int i=0;i<n;i++){
...
int main(){
    ...
    string name = "original_board";
    Board original_board(n,name);

    // int board[n][n]; //비숍을 배치할 수 없다면 0, 배치할 수 있으면 1, 비숎이 배치됐다면 2, 배치된 비숍때문에 배치할 수 없다면 3
    //보드 입력 받기
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            cin>>original_board.board[i][j];
        }
    }
    ...
    for(int x=0;x<n;x++){
        for(int y=0; y<n;y++){
            ...
            cout<<"original_board.board[x][y] : "<<original_board.board[x][y]<<endl;
